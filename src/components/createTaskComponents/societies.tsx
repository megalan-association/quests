...
export default function Societies({ setSocieties }: Props) {
  const [primaryKey, setPrimaryKey] = React.useState<Society | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    if (collabKeys.size > 0) {
      ...
      Array.from(collabKeys)
        .map(
          (collabName: string) =>
            allSocieties.filter((society) => society.name === collabName)[0],
        )
        .forEach((collab) => collab && societies.push(collab.id));
    }
  ...
  return (
    <div className="flex w-[80vw] flex-col gap-4 md:w-[350px]">
      <Dropdown classNames={{ base: "w-[80vw] md:w-[350px]" }}>
        ...
        <DropdownMenu
          ...
          onSelectionChange={(key) => {
            setPrimaryKey(joinedSocieties[Object.values(key)[0]]);
          }}
        >
          {joinedSocieties.map((society, index) => (
            <DropdownItem key={index}>{society.name}</DropdownItem>
          ))}
        </DropdownMenu>
      </Dropdown>
      ...
      {isCollab && (
        <Dropdown classNames={{ base: "w-[80vw] md:w-[350px]" }}>
          <DropdownTrigger>
            <Button variant="bordered">
              {displayCollabKeys.length > 0
                ? displayCollabKeys
                : "Choose ONE or MORE collab societies"}
            </Button>
          </DropdownTrigger>
          <DropdownMenu
          ...
          >
            {allSocieties
              .filter((society) => society.id !== primaryKey?.id)
              .map((society) => (
                <DropdownItem key={society.name}>{society.name}</DropdownItem>
              ))}
          </DropdownMenu>
        </Dropdown>
      )}
